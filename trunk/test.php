<?php
echo "Initializing:". xcb_init()."\n";
echo "Root Width: ".xcb_root_width_in_pixels(1)."\n";

$tmp = xcb_list_extensions();
var_export($tmp);
foreach($tmp as $name){
	$info = xcb_query_extension($name);
	echo $name .":".$info['first_event']."\n";
	//var_export(xcb_query_extension($name));
}
//exit();

$newid = xcb_generate_id();
xcb_create_window($newid, xcb_root_id(), 200, 200, 10, 10, 6);
xcb_configure_window_events($newid, array(1048576,  524288));
xcb_map_window($newid);
xcb_flush();
sleep(1);
...
//} xcb_config_window_t;
xcb_configure_window($newid, array(1, 2, 4, 8), array(150, 150, 300, 300));
xcb_flush();
sleep(1);
var_export(xcb_get_geometry($newid));
...
$intColorMap = xcb_get_default_colormap();
//xcb_create_colormap($intColorMap, $newid);
echo "Colormap: {$intColorMap}\n";
//$red = xcb_alloc_color($intColorMap, "red", 255, 0, 0);
$red = xcb_alloc_named_color($intColorMap, "Red");
echo "Red: {$red}\n";

$gcid = xcb_generate_id();
echo "GC ID: {$gcid}\n";
xcb_create_gc($newid, $gcid, array(4, 8), array($red, $red));

xcb_poly_fill_rectangle($newid, $gcid, 10, 10, 20, 20);
